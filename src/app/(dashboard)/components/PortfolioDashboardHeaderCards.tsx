        ...
        <div>
            <section className={"min-h-[150px] main-gradient w-full shadow-lg rounded-4xl overflow-hidden font-medium"}>
                <div className={"text-2xl p-5 "}>
                    ...
                    <p className={"text-light-secondary dark:text-dark-tertiary text-lg"}>Your total balance</p>
                </div>
            </section>
                ...
                {values.map( (item,index) => {
                    return (
                        <div key={index} className={"w-full min-h-[150px] shadow-lg rounded-4xl overflow-hidden bg-light-bg-secondary dark:bg-dark-bg-tertiary"}>
                            <div className={`text-2xl p-5 `}>
                                <div className={`${index === 2 ? item.value > 0 ? "text-green-600 bg-[hsl(80,60%,90%)]" : item.value < 0 ? "text-light-error-text dark:text-dark-error-text bg-[hsl(0,60%,90%)] dark:bg-dark-error-bg" : "text-light-main dark:text-dark-main bg-[hsl(260,60%,90%)] dark:bg-[hsl(258,20%,65%)] " : "text-light-main dark:text-[hsl(258,20%,65%)] bg-[hsl(260,60%,90%)] dark:bg-[hsl(258,25%,35%)]" } w-[45px] h-[45px] rounded-2xl flex justify-center items-center `}>
                                ...
                                ) : <p className={`mt-3 text-2xl ${index === 2 && item.value > 0 ? "text-green-600" : index === 2 && item.value < 0 ? "text-light-error-text dark:text-dark-error-text" : "" }`}>{ index === 2 ? item.value > 0 ? `+${item.value} ${currency} (+${item.percent}%)` : `${item.value} ${currency} (${item.percent}%)` : index === 0 ? `${item.value} ${currency}` : item.value }</p>}
                                <p className={"text-dark-text-tertiary dark:text-dark-text-secondary text-lg"}>{item.label}</p>
                            </div>
                        </div>
                    );
                })}
