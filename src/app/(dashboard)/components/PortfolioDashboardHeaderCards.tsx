...
export default function PortfolioDashboardHeaderCards({totalBalance, values, isLoading, currency}: {totalBalance: number; values: {label:string, value:number, percent?:number;}[]; isLoading:boolean; currency:string } ) {
    return(
        <div>
            <section className={"min-h-[150px] bg-light-main dark:bg-dark-main w-full shadow-lg rounded-4xl overflow-hidden font-medium"}>
                <div className={"text-2xl p-5 "}>
                    <div className={`w-[40px] h-[40px] rounded-2xl flex justify-center items-center bg-[hsl(266,56%,80%)] text-white `}>
                        <FaWallet size={25}/>
                    </div>
                    {isLoading ? (
                        <FaSpinner className="animate-spin text-4xl mx-auto text-light-main dark:text-dark-main" />
                    ) : <p className={`text-light-text-secondary mt-3 text-2xl `}>{totalBalance.toFixed(2)} {currency}</p>}
                    <p className={"text-[#E9D9F9] text-lg"}>Your total balance</p>
                </div>
            </section>
            <section className={`grid grid-cols-1 320:grid-cols-2 lg:grid-cols-3 gap-5 font-medium transition-all mt-5 tracking-tighter`}>
                {values.map( (item,index) => {
                    return (
                        <div key={index} className={"w-full min-h-[150px] shadow-lg rounded-4xl overflow-hidden bg-light-bg-secondary dark:bg-dark-bg-secondary"}>
                            <div className={`text-2xl p-5 `}>
                                <div className={`${index === 2 ? item.value > 0 ? "text-green-500 bg-[hsl(80,60%,90%)]" : item.value < 0 ? "text-light-error-text bg-[hsl(0,60%,90%)]" : "text-light-main bg-[hsl(260,60%,90%)]" : "text-light-main bg-[hsl(260,60%,90%)]" } w-[45px] h-[45px] rounded-2xl flex justify-center items-center `}>
